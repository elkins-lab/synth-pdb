...
def create_buried_exposed_system():
    ...
    atoms.append(
        struc.Atom([0, 0, 0], atom_name="CA", res_name="ALA", res_id=1, chain_id="A", element="C")
    )
    # Surround it with "shielding" atoms (Res 3, 4, 5, 6, 7, 8)
    # Positions +/- 3.5 Angstroms in X, Y, Z
    offsets = [[3.5, 0, 0], [-3.5, 0, 0], [0, 3.5, 0], [0, -3.5, 0], [0, 0, 3.5], [0, 0, -3.5]]
    for i, off in enumerate(offsets):
        res_id = 10 + i
        atoms.append(
            struc.Atom(
                off, atom_name="CA", res_name="ALA", res_id=res_id, chain_id="A", element="C"
            )
        )

    # Residue 2: Far away (Exposed)
    atoms.append(
        struc.Atom([50, 0, 0], atom_name="CA", res_name="ALA", res_id=2, chain_id="A", element="C")
    )
