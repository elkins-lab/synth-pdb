...
# We use the midpoint and a +/-0.010 A delta to stay robust across CI environments.
...
class TestScientificRigor(unittest.TestCase):
    ...
    def test_csi_secondary_structure_discrimination(self):
        """Verify that Calpha Chemical Shift Index (CSI) correctly discriminates between
        alpha-helix and beta-strand conformations generated by synth-pdb.

        EDUCATIONAL NOTE - The Chemical Shift Index (CSI):
        --------------------------------------------------
        The Calpha chemical shift is acutely sensitive to backbone dihedral angles
        (phi/psi). In an alpha helix (phi~-60, psi~-45), the Calpha nucleus is slightly
        deshielded relative to random coil, giving a *positive* secondary shift
        (Delta_Calpha > 0). In a beta sheet (phi~-120, psi~+120), the opposite applies.

        Wishart & Sykes (1994) established the empirical threshold:
          - Delta(Calpha) > +0.7 ppm -> Helix
          - Delta(Calpha) < -0.7 ppm -> Sheet

        This test validates that synth-pdb's generator produces backbone geometries
        whose Calpha environments are chemically consistent with the requested secondary
        ...
        from synth_nmr.chemical_shifts import calculate_csi, predict_empirical_shifts
        ...
        # 1. Helix: mean Calpha CSI must be clearly positive
        self.assertGreater(
            helix_mean,
            0.7,
            f"Helix mean Calpha CSI ({helix_mean:+.3f} ppm) should be > +0.7 ppm. "
            "This suggests backbone phi/psi angles are not in the helical region.",
        )

        # 2. Strand: mean Calpha CSI must be clearly negative
        self.assertLess(
            strand_mean,
            -0.7,
            f"Strand mean Calpha CSI ({strand_mean:+.3f} ppm) should be < -0.7 ppm. "
        ...
        self.assertGreater(
            helix_fraction,
            0.80,
            f"Only {helix_fraction:.0%} of helix residues have Calpha CSI > +0.7 ppm "
        ...
        self.assertGreater(
            strand_fraction,
            0.80,
            f"Only {strand_fraction:.0%} of strand residues have Calpha CSI < -0.7 ppm "
            f"(expected >=80%). The backbone conformation is inconsistent.",
        )

        # 4. The two distributions must be well separated (Deltamean >= 2.0 ppm)
