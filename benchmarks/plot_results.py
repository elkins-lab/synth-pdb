...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="List of input CSV files (e.g., machine1.csv machine2.csv)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="benchmarks/bench_plot_comparison.png",
    ...
    for f in args.inputs:
        ...
        df["hardware_label"] = (
            f"{df['processor'][0]} ({df['cpu_threads'][0]}T, {df['ram_total_gb'][0]}GB)"
        )
        all_dfs.append(df)
    ...
    sns.barplot(
        x="N", y="throughput_batched", hue="hardware_label", data=df_all, ax=ax1, palette=palette
    )
    ...
    sns.lineplot(
        x="N", y="speedup", hue="hardware_label", data=df_all, ax=ax2, marker="o", palette=palette
    )
