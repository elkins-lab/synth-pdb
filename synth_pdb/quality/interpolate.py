...
def interpolate_structures(
    ...
    if len(start_ca) != len(end_ca):
        raise ValueError(
            f"Structures have different lengths: {len(start_ca)} vs {len(end_ca)}. Interpolation requires same length."
        )
...
def _reconstruct_backbone(phi: np.ndarray, psi: np.ndarray, omega: np.ndarray) -> np.ndarray:
    ...
    def pos(
        p1: np.ndarray, p2: np.ndarray, p3: np.ndarray, bl: float, ba: float, di: float
    ) -> np.ndarray:
    ...
    for i in range(1, length):
        ...
        # Place O(i)
        coords[idx + 3] = pos(
            coords[idx], coords[idx + 1], coords[idx + 2], BOND_LENGTH_C_O, ANGLE_CA_C_O, np.pi
        )
...
def _write_simple_pdb(coords: np.ndarray, res_names: list[str], path: str) -> None:
    ...
    with open(path, "w") as f:
        ...
        for i, res_name in enumerate(res_names):
            ...
            # N
            f.write(
                f"ATOM  {atom_idx:>5d}  N   {res_name:>3s} A{i + 1:>4d}    {coords[idx][0]:8.3f}{coords[idx][1]:8.3f}{coords[idx][2]:8.3f}  1.00  0.00           N\n"
            )
            atom_idx += 1
            # CA
            f.write(
                f"ATOM  {atom_idx:>5d}  CA  {res_name:>3s} A{i + 1:>4d}    {coords[idx + 1][0]:8.3f}{coords[idx + 1][1]:8.3f}{coords[idx + 1][2]:8.3f}  1.00  0.00           C\n"
            )
            atom_idx += 1
            # C
            f.write(
                f"ATOM  {atom_idx:>5d}  C   {res_name:>3s} A{i + 1:>4d}    {coords[idx + 2][0]:8.3f}{coords[idx + 2][1]:8.3f}{coords[idx + 2][2]:8.3f}  1.00  0.00           C\n"
            )
            atom_idx += 1
            # O
            f.write(
                f"ATOM  {atom_idx:>5d}  O   {res_name:>3s} A{i + 1:>4d}    {coords[idx + 3][0]:8.3f}{coords[idx + 3][1]:8.3f}{coords[idx + 3][2]:8.3f}  1.00  0.00           O\n"
            )
            atom_idx += 1
        f.write("TER\nEND\n")
