...
translation. If you rotate a protein by 90deg, the (x, y, z) values change
...
-----------------------------------------------------------------------------
  ...
  Nodes  - one per residue, identified by Calpha position.
  Edges  - bidirectional between every Calpha pair within 8 A of each other.
           8 A is a biologically meaningful cutoff: it captures direct
  ...
  Node features  (8-dimensional):
    [0] sin(phi)          Backbone torsion - periodic encoding avoids the
    [1] cos(phi)          +/-180deg discontinuity. Alpha helices cluster at
    [2] sin(psi)          phi ~ -60deg, psi ~ -45deg. Beta strands at phi ~ -120deg,
    [3] cos(psi)          psi ~ +120deg. The GNN learns these clusters without
                        us hard-coding them.
  ...
  Edge features  (2-dimensional):
    [0] Calpha-Calpha distance  Physical distance in A.  Nearby residues interact
...
def build_protein_graph(pdb_content: str, ca_distance_threshold: float = 8.0) -> Any:
    ...
    Args:
        ...
        ca_distance_threshold: Maximum Calpha-Calpha distance (A) for an edge to be
            created. 8 A is a standard contact-map cutoff in structural biology.
    ...
    Raises:
        ValueError: If fewer than 2 residues with Calpha atoms are found.
    ...
    if len(residues) < 2:
        ...
        raise ValueError(
            f"Found only {len(residues)} Calpha atom(s) in PDB. "
            "Need at least 2 residues to build a graph."
        )

    n = len(residues)
    logger.debug("Building graph for %d residues (threshold=%.1f A)", n, ca_distance_threshold)

    # ------------------------------------------------------------------
    # Step 2 - Build node feature matrix  x  of shape [N, 8]
    # ------------------------------------------------------------------
    # Each row describes one residue.  The features are chosen to capture:
    #   * Backbone geometry  (phi/psi dihedrals - most discriminative signal)
    ...
    # Collect Calpha coordinates - needed for the pairwise distance matrix.
    ...
    # Raw B-factors vary across structures (e.g. 5-80 A^2).  We normalise
    ...
    # Because -180deg and +180deg are the SAME conformation, but numerically
    # very far apart.  Encoding as (sin theta, cos theta) makes the representation
    ...
    # Compute pairwise Calpha distance matrix using broadcasting: O(N^2) memory
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                ...
                continue
            d = dist_matrix[i, j]
            # Biologically meaningful cutoff (8 A) captures H-bonds and VdW.
    ...
    if not src_list:
        # Safety fallback: if all Calpha atoms happened to be > 8 A apart
        ...
        logger.warning(
            "No edges found within %.1f A threshold - adding sequential backbone edges.",
...
def _parse_backbone(pdb_content: str) -> list[dict]:
    """Extract per-residue Calpha coordinates, B-factors, and backbone dihedrals
    ...
    by character columns rather than delimiters (like commas).
      ...
      cols 31-38  X coordinate (A)
      cols 39-46  Y coordinate (A)
      cols 47-54  Z coordinate (A)
      cols 61-66  B-factor
    ---------------------------------------------------------------------
    """
    ...
    # phi (phi) = dihedral(C_{i-1}, N_i, Calpha_i, C_i)
    # psi (psi) = dihedral(N_i, Calpha_i, C_i, N_{i+1})
    #
    # These are undefined for the first (phi) and last (psi) residues, where
...
def build_protein_graphs_from_batch(batch: Any, ca_distance_threshold: float = 8.0) -> list[Any]:
    ...
    Args:
        ...
        ca_distance_threshold: Spatial cutoff (A) for creating edges between
            residue nodes.
    ...
    # every structure in the batch to calculate phi/psi dihedrals.
    ...
    for i in range(b):
        ...
        # These dihedrals (phi and psi) are the most important descriptors of
        # local secondary structure quality (Ramachandran status).
        # phi (phi) = C(i-1)-N(i)-CA(i)-C(i)
        # psi (psi) = N(i)-CA(i)-C(i)-N(i+1)
        for res_idx in range(length):
            ...
            # Neural networks struggle with discontinuities (+/-180deg). By encoding
...
def _dihedral(p1: np.ndarray, p2: np.ndarray, p3: np.ndarray, p4: np.ndarray) -> float:
    ...
    The dihedral is the angle between the planes (b1,b2) and (b2,b3):
        n1 = b1 x b2   <- normal to plane 1
        n2 = b2 x b3   <- normal to plane 2
        theta  = arccos(n1 * n2)
